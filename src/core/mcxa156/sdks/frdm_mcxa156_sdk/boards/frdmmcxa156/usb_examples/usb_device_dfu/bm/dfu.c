...
usb_status_t USB_DeviceDfuDemoCallback(class_handle_t handle, uint32_t event, void *param)
{
    ...
    switch (event)
    {
        ...
        case kUSB_DeviceDfuEventDownLoad:
            if ((0U != controlRequest->isSetup) && (controlRequest->setup->wLength > 0U))
            {
...
void USB_DeviceDfuSwitchMode(void)
{
    ...
    SIM->PSWR3   = 0U;
    SIM->PSWR2   = 0U;
    s_DfuAddress = ((uint32_t)USB_DFU_APP_ADDRESS) >> 1U;
    INTC->VBA    = (s_DfuAddress >> 8U);
    asm(move.l s_DfuAddress, N);
    asm(jmp(N));
#else
    s_DfuAddress = (uint32_t)(USB_DFU_APP_ADDRESS);
    ...
    (void)USB_DeviceClassDeinit(CONTROLLER_ID);
#endif
    SCB->VTOR = s_DfuAddress;
    newSP     = ((uint32_t *)s_DfuAddress)[0U];
    newPC     = ((uint32_t *)s_DfuAddress)[1U];
...
static void USB_DeviceDfuManifest(void)
{
    ...
    {
        s_DfuCrcValue = 0U;
        remainingLen  = s_UsbDeviceDfuDemo.dfuFirmwareSize - 4U;
#if defined(__DSC__) || defined(__CW__)
        startAddress = (uint8_t *)(USB_DFU_APP_ADDRESS_DATA_MEMORY_MAP * 2U);
#else
